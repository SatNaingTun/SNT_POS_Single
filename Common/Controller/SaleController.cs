...
namespace SNT_POS_Common.Controller
{
   public class SaleController
   {
       ...
       CustomerController customerControl = new CustomerController();
        ...
        {
            string query = "SELECT tbl_Sale.ID,tbl_Sale.StockID, tbl_Sale.SaleTypeID, tbl_Sale.Price, tbl_Sale.Quantity as Quantity, tbl_Sale.Amount, tbl_Sale.Discount, tbl_Sale.UserID, tbl_Sale.NetAmount, tbl_Sale.SessionID, tbl_Sale.SaleDateTime, tbl_Sale.VouncherID, tbl_SaleType.SaleTypeName, tbl_Stock.StockName, Users.UserName,tbl_Sale.CustomerID,tbl_Customer.Name as CustomerName,tbl_Customer.Phone as CustomerPhone FROM ((((tbl_Sale INNER JOIN tbl_SaleType ON tbl_Sale.SaleTypeID = tbl_SaleType.ID) INNER JOIN tbl_Stock ON tbl_Sale.StockID = tbl_Stock.ID) INNER JOIN Users ON tbl_Sale.UserID = Users.ID) left join tbl_Customer on tbl_Sale.CustomerID=tbl_Customer.ID) where tbl_Sale.ID=@id and tbl_Sale.isDeleted=false";
        ...
        private Vouncher_Item toVouncherItem(DataTable dt)
        {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    item.VouncherID = dt.Rows[0].Field<string>("VouncherID");

                    item.stock = stockControl.getById(dt.Rows[0].Field<int>("StockID"));
                    item.stock.Price = dt.Rows[0].Field<decimal>("Price");
